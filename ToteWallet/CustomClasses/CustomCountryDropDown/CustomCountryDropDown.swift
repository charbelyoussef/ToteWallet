...
@objc protocol CustomCountryDropDownDelegate {
    @objc optional func dropDownCountryDidSelect(contentReturned: [Country], tag:Int)
}

class CustomCountryDropDown: FadingViewController , UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    private static var dropDown:CustomCountryDropDown?
    var selectedItems = [Country]()
    
    var delegate: CustomCountryDropDownDelegate?
    ...
    // MARK: Class Methods
    class func open(title:String, data:[Country], selectedItems:[Country]?, delegate:CustomCountryDropDownDelegate, multipleSelection: Bool, tag:Int){
        dropDown = CustomCountryDropDown(nibName: "CustomCountryDropDown", bundle: Bundle.main)
    ...
    @objc func close() {
        dropDownTag = -1
        if CustomCountryDropDown.dropDown != nil {
            CustomCountryDropDown.dropDown?.dismiss(animated: true, completion: nil)
        }
    }
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        if searchText == "" {
            ...
            content = content.filter { ($0.codeWithName?.contains(searchText))! }
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = content[indexPath.row].codeWithName
    ...
    @objc func dismissAfterSelection(){
        self.delegate?.dropDownCountryDidSelect?(contentReturned: selectedItems, tag: dropDownTag)
        self.close()
    }
