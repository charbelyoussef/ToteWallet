...
@objc protocol CustomDropDownDelegate {
    @objc optional func dropDownDidSelect(contentReturned: [String], tag:Int)
}

class CustomDropDown: FadingViewController , UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    private static var dropDown:CustomDropDown?
    ...
    //Mark Class Methods
    class func open(title:String, data:[String], selectedItems:[String]?, delegate:CustomDropDownDelegate, multipleSelection: Bool, tag:Int){
        dropDown = CustomDropDown(nibName: "CustomDropDown", bundle: Bundle.main)
    ...
    @objc func close() {
        dropDownTag = -1
        if CustomDropDown.dropDown != nil {
            CustomDropDown.dropDown?.dismiss(animated: true, completion: nil)
        }
    }
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        if searchText == "" {
            ...
            content = content.filter { $0.contains(searchText) }
        }
    ...
    @objc func dismissAfterSelection(){
        self.delegate?.dropDownDidSelect?(contentReturned: selectedItems, tag: dropDownTag)
        self.close()
    }
