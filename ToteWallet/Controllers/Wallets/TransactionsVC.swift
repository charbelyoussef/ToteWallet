...
class TransactionsVC : UIViewController, UITableViewDelegate, UITableViewDataSource, UINavigationControllerDelegate, CalendarPickerDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        tvContent.customRegisterForCell(identifiers: ["TransactionsVCDefaultCell"])

        let fromDate = Date.daysFromNowWithoutTime(days: -7)
...
//    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:TransactionsVCDefaultCell = tableView.dequeueReusableCell(withIdentifier: "TransactionsVCDefaultCell", for: indexPath) as! TransactionsVCDefaultCell
        
        cell.configureCell(indexPath:indexPath, transaction: contentArray?[indexPath.row])
...
//        btnReceiverCountryCode.setTitle(countries.first?.codeWithName, for: .normal)
    ...
    @objc func expandDescription(sender: UIButton!) {
        ...
        if let cell = tvContent.cellForRow(at: indexPath) as? TransactionsVCDefaultCell {
    ...
    func fetchTransactions(){
        showProgressBar()
        User.shared.getTransactionsWithFilter(delegate: self,
...
extension TransactionsVC: CustomCountryDropDownDelegate {
...
extension TransactionsVC: CNContactPickerDelegate {
...
extension TransactionsVC: UIPickerViewDelegate, UIPickerViewDataSource {
...
extension TransactionsVC : OBSRemoteDataDelegate {
    func hasFinishedLoadingData(status: OBSRemoteDataStatus.Name, message: String) {
        if status == .HTTPSuccess {
            contentArray = CoreDataService.getTransactions()
            tvContent.reloadData()
            hideProgressBar()
        }
    ...
    func hasFinishedLoadingDataWithError(error: Error?) {
        ...
        if error != nil, error?.localizedDescription != "" {
            ...
            showAlertWithCompletion(message: "Failed To Get Transactions", okTitle: "Retry", cancelTitle: "Cancel") {
