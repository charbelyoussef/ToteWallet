...
class ProgramRegistrationVC: UIViewController {
...
//        btnCountryCode.setTitle(countries.first?.codeWithName, for: .normal)
        ...
        tfProgramName.text = program?.name
        ...
        tfTotalCost.text = program?.totalIn
        
        tfQuantity.tag = TextFieldsTags.quantity.rawValue
    ...
    func openProfile(){
        self.performSegueIfPossible(identifier: "programRegistrationToProfile")
    }
...
//            let keyboardHeight = keyboardFrame.cgRectValue.height
            if Enums.DeviceType.IS_IPHONE_5 {
                ...
                self.view.frame.origin.y = 0 - Constants.UIElements.iphoneXOffset - 50
            }
            else if Enums.DeviceType.IS_IPHONE_XsMax {
                self.view.frame.origin.y = 0 - Constants.UIElements.iphoneXOffset - 100
            }
...
//        initPicker(tag: Enums.Pickers.Country.rawValue)
    ...
    @IBAction func btnPurchaseAction(_ sender: Any) {
        if program != nil, let programId = program?.id,
           let phoneNumber = tfPhoneNumber.text, phoneNumber != "",
           ...
           let quantity = tfQuantity.text, quantity != "" {
            ...
            showProgressBar()
            WalletStore.shared.registerProgram(delegate: self,
...
extension ProgramRegistrationVC : UITextFieldDelegate {
    ...
    func textFieldShouldEndEditing(_ textField: UITextField) -> Bool {
        ...
        case TextFieldsTags.quantity.rawValue:
            if let subAmount = textField.text, subAmount != "",
               let onlineFees = program?.totalIn, onlineFees != "",
               let subAmountInt = Int(subAmount),
               let onlineFeesInt = Int(onlineFees) {
                let totalAmount = Int(subAmountInt)*Int(onlineFeesInt)
                tfTotalCost.text = "\(totalAmount)"
            }
            return true
        default:
            return true
...
extension ProgramRegistrationVC: CNContactPickerDelegate {
...
extension ProgramRegistrationVC: CustomCountryDropDownDelegate {
...
extension ProgramRegistrationVC: UIPickerViewDelegate, UIPickerViewDataSource {
...
extension ProgramRegistrationVC : OBSRemoteDataDelegate {
